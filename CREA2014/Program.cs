    ...
    public static class Program
    {
        ...
        public class UPnP3
        {
            public UPnP3(IPAddress _machineIpAddress, IPAddress _gatewayIpAddress)
            {
                ...
                {
                    string location = MSearch(service);
                    ...
                    if (dv != null)
                    {
                        int serviceIndex = deviceDescription.IndexOf(service);
                        if (serviceIndex == -1)
                            continue;

                        controlURL = GetTagValue("controlURL", deviceDescription.Substring(serviceIndex));
                        serviceType = service;
            ...
            public string GetGenericPortMappingEntry(int portMappingIndex)
            {
                ...
                string newExternalPort = GetTagValue("NewExternalPort", result);
                string newProtocol = GetTagValue("NewProtocol", result);
                string newInternalPort = GetTagValue("NewInternalPort", result);
                string newInternalClient = GetTagValue("NewInternalClient", result);
                string newEnabled = GetTagValue("NewEnabled", result);
                string newPortMappingDescription = GetTagValue("NewPortMappingDescription", result);
                string newLeaseDuration = GetTagValue("NewLeaseDuration", result);
            }

            public class GenericPortmappingEntry
            {
                public GenericPortmappingEntry(string _newRemoteHost, ushort _newExternalPort, string _newProtocol, ushort _newInternalPort, IPAddress _newInternalClient, string _newEnabled, string _newPortMappingDescription, int _newLeaseDuration)
                {
                ...
                public string NewEnabled { get; private set; }
